...
OUTPUT_ROOT = Path('gru_rolling_session_outputs')
...
def main():
    ...
    while test_end <= n_samples:
        ...
        model = StandardGRUModel(input_size, HIDDEN_SIZE, output_size, NUM_LAYERS).to(device)
        if session > 0:
            previous_model_path = OUTPUT_ROOT / f'session_{session - 1:03d}' / 'gru_model.pth'
        ...
        torch.save(model.state_dict(), session_dir / 'gru_model.pth')
    ...
    np.savez(
        OUTPUT_ROOT / 'gru_rolling_metrics.npz',
