...
OUTPUT_ROOT = Path('lstm_rolling_session_outputs')
...
def save_prediction_plot(predictions, targets, labels, title, save_path, n_display):
    plot_predictions(
        predictions,
        targets,
        title=title,
        labels=labels,
        n_display=min(n_display, len(predictions)),
    )
    plt.tight_layout()
...
def save_session_predictions(predictions, targets, target_names, save_path):
    df_out = pd.DataFrame()
    for i, name in enumerate(target_names):
        df_out[f'true_{name}'] = targets[:, i]
        df_out[f'pred_{name}'] = predictions[:, i]
    df_out.to_csv(save_path, index=False)


def compute_metrics(predictions, targets, target_names):
    n_outputs = targets.shape[1]

    r2_vals = [r2_score(targets[:, i], predictions[:, i]) for i in range(n_outputs)]
    mae_vals = [mean_absolute_error(targets[:, i], predictions[:, i]) for i in range(n_outputs)]
    rmse_vals = [np.sqrt(mean_squared_error(targets[:, i], predictions[:, i])) for i in range(n_outputs)]
    smape_vals = [sym_mean_absolute_percentage_error(targets[:, i], predictions[:, i]) for i in range(n_outputs)]
...
def main():
    ...
    while test_end <= n_samples:
        ...
        model = StandardLSTMModel(input_size, HIDDEN_SIZE, output_size, NUM_LAYERS).to(device)
        if session > 0:
            previous_model_path = OUTPUT_ROOT / f'session_{session - 1:03d}' / 'lstm_model.pth'
        ...
        predictions, targets = predict_deterministic(model, test_loader, scaler_y, device)
        ...
        metrics = compute_metrics(predictions, targets, target_names)
        metrics.update({
            'session': session,
        ...
        torch.save(model.state_dict(), session_dir / 'lstm_model.pth')
    ...
    np.savez(
        OUTPUT_ROOT / 'lstm_rolling_metrics.npz',
