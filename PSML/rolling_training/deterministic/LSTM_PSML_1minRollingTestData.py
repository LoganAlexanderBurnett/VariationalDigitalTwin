...
while test_end <= n_samples:
    ...
    # model setup
    if session == 0:
        model = StandardLSTMModel(in_f, hidden, out_f, n_layers).to(device)
    else:
        model.load_state_dict(
            torch.load(f"lstm_session{session-1}.pth", map_location=device)
        )
    ...
    print(f" Inference took {infer_time}s")
    ...
    # save model
    torch.save(model.state_dict(), f"lstm_session{session}.pth")
