    ...
    for epoch in range(num_epochs):
        ...
        for inputs, targets in train_loader:
            ...
            if isinstance(model_outputs, tuple):
                outputs, kl_loss = model_outputs
            else:
                outputs = model_outputs
                kl_loss = torch.tensor(0.0, device=outputs.device)
        ...
        with torch.no_grad():
            for val_inputs, val_targets in val_loader:
                ...
                model_outputs = model(val_inputs)
                if isinstance(model_outputs, tuple):
                    val_outputs, val_kl_loss = model_outputs
                else:
                    val_outputs = model_outputs
                    val_kl_loss = torch.tensor(0.0, device=val_outputs.device)
