...
def _build_rolling_predictions_pdf_path(args) -> Path:
...
def _build_rolling_metrics_png_path(args) -> Path:
    return Path(args.results_dir) / (
        f"{args.results_prefix}-{args.save_model}-batch_size={args.train_runs}-seq_len={args.seq_len}-metrics.png"
    )


def _create_iteration_title_figure(iteration: int, test_files: list[str]):
    fig = plt.figure(figsize=(8.27, 11.69))
    fig.text(0.5, 0.72, "Rolling Prediction Report", ha='center', va='center', fontsize=24, weight='bold')
    fig.text(0.5, 0.62, f"Iteration {iteration}", ha='center', va='center', fontsize=20)
    fig.text(0.5, 0.52, f"Predictions for {len(test_files)} run(s)", ha='center', va='center', fontsize=14)
    fig.text(0.5, 0.40, "\n".join(test_files), ha='center', va='center', fontsize=11)
    return fig
...
def _run_rolling_experiment(
    ...
    iteration_metrics = []
    per_run_metrics = []
    predictions_pdf_path = _build_rolling_predictions_pdf_path(args)
    metrics_png_path = _build_rolling_metrics_png_path(args)
    ...
    with PdfPages(predictions_pdf_path) as pdf:
        for iteration, start in enumerate(range(block_size, total_runs, block_size), start=1):
            ...
            title_fig = _create_iteration_title_figure(iteration, test_files)
        ...
        plt.legend()
        plt.tight_layout()
        summary_fig.savefig(metrics_png_path, dpi=300, bbox_inches='tight')
        pdf.savefig(summary_fig)
        plt.close(summary_fig)
    print(f'Saved rolling prediction report to {predictions_pdf_path}')
    print(f'Saved rolling metrics summary PNG to {metrics_png_path}')
