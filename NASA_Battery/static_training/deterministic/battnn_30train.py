...
def plot_all_data_with_dates(npz_dir='./data/', num_plots=5):
    """Overlay all test runs, each with twin y-axes and its start date."""
    D = NPZData(npz_dir)
    count = 0
    for c, v, d in D.yield_test_data():
        count += 1
        ...
        ax1.plot(c, color='g', label='Current')
        ax1.set_ylabel('Current (A)', color='g')
        ax1.set_xticks([0])
        ax1.set_xticklabels([d], rotation=45, ha='right')

        ax2.plot(v, color='r', label='Voltage')
        ax2.set_ylabel('Voltage (V)', color='r')
